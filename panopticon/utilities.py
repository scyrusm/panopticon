...
def get_tcr_doublet_likelihood(tcrs,
                               ...
                               n_tra=2,
                               n_trb=2):
    # from panopticon.utilities import multinomial_test
    ...
    df.name = 'counts'
    df = df.reset_index()
    # df.columns = ['TCR', 'counts']
    print(len(samples), df.shape)
        ...
        if n_tra == 2 and n_trb == 2:
            for alpha in row['TRA'].split('-'):
                ...
                for beta in row['TRB'].split(
                    ...
                    if x1 > 0 and x2 > 0:
                        p = multinomial_test(x1, x2, x12, n, 0.05)
                    else:
                        p = 0
                ...
                if beta == '*':
                    ...
                    df1 = df.query('sample==@sample').query(
                        'TRA==@alpha').query('TRB==@beta').query(
                            'N_TRA==1').query('N_TRB==1').sort_values('count')
                    df2 = df.query('sample==@sample').query(
                        'TRA==@alpha_c').query('N_TRA==1').query(
                            'N_TRB==1').sort_values('count')
                if df1.shape[0] == 1:
                    x1 = df1['counts'].values[0]
                elif df1.shape[0] > 1:
                    display(df1)
                ...
                if alpha == '*':
                    ...
                    df1 = df.query('sample==@sample').query(
                        'TRA==@alpha').query('TRB==@beta').query(
                            'N_TRA==1').query('N_TRB==1')
                    df2 = df.query('sample==@sample').query(
                        'TRB==@beta_c').query('N_TRA==1').query('N_TRB==1')
                else:
                    alpha_c = '*'
                    df1 = df.query('sample==@sample').query(
                        'TRB==@beta').query('N_TRA==1').query('N_TRB==1')
                    df2 = df.query('sample==@sample').query(
